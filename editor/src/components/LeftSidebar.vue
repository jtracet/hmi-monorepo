  <template>
    <div v-bind="$attrs">
      ...
      <div class="w-64 bg-gray-100 p-4 overflow-y-auto space-y-2 border-r">
                <!-- Controls -->
        <div v-if="palette.controls.length">
          <h2 class="font-semibold text-lg mb-2">Controls</h2>
          <button
            v-for="s in palette.controls"
            :key="s.key"
            draggable="true"
        ...
        <!-- Indicators -->
        <div v-if="palette.indicators.length" class="mt-4">
          <h2 class="font-semibold text-lg mb-2">Indicators</h2>
          <button
            v-for="s in palette.indicators"
            :key="s.key"
            draggable="true"
            @dragstart="e => e.dataTransfer?.setData('shape', s.key)"
            class="block w-full px-3 py-1.5 bg-white rounded shadow text-left hover:bg-gray-50 active:scale-95 transition"
        ...
        <!-- Decorations -->
        <div v-if="palette.decorations.length" class="mt-4">
          <h2 class="font-semibold text-lg mb-2">Decorations</h2>
          <button
            v-for="s in palette.decorations"
            :key="s.key"
            draggable="true"
            @dragstart="e => e.dataTransfer?.setData('shape', s.key)"
            class="block w-full px-3 py-1.5 bg-white rounded shadow text-left hover:bg-gray-50 active:scale-95 transition"
  ...
  Object.entries(ElementRegistry).forEach(([key, ctor]) => {
    const cat = (ctor as any).category || 'controls'  // default
    palette[cat].push({
      key,
